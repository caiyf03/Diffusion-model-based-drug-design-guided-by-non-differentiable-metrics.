...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        print("[INFO]lightening_modules: the mode is:", mode)
        self.mode = mode
        ...
        self.dataset_info = dataset_params[dataset] # we are using the dataset_params['crossdock'] in constants.py
        ...
        self.lig_type_decoder = self.dataset_info['atom_decoder']  # we are using the dataset_params['crossdock'] in constants.py
        ...
        self.atom_nf = len(self.lig_type_decoder)  # we are using the dataset_params['crossdock'] in constants.py
        ...
        #print("Lightning module state dict keys:")
    ...
    def get_ligand_and_pocket(self, data):
        ligand = {
            'x': data['lig_coords'].to(self.device, FLOAT_TYPE), # 配体中各个原子的三维坐标
            'one_hot': data['lig_one_hot'].to(self.device, FLOAT_TYPE), # 从数据集中获取的配体原子类型的独热编码张量
            'size': data['num_lig_atoms'].to(self.device, INT_TYPE), # 配体中原子的数量
            'mask': data['lig_mask'].to(self.device, INT_TYPE), # 配体的掩码，标记哪些原子是有效的，哪些是无效的
        }
    ...
    ####################################### This is for DDPO:##################################
    def compute_reward(self, xh_lig_hat, ligand, pocket):
        """
        计算奖励函数。
        :param xh_lig_hat: 生成的配体数据
        :param ligand: 配体信息
        :param pocket: 口袋信息
        :return: 奖励值
        """
        # 根据下游任务设计奖励函数
        # 例如：计算配体与口袋的结合亲和力、配体的药物相似性等
        reward = self.reward_function(xh_lig_hat, ligand, pocket)
        return reward

    def compute_ddpo_loss(self, xh_lig_hat, reward):
        """
        计算 DDPO 的策略梯度损失。
        :param xh_lig_hat: 生成的配体数据
        :param reward: 奖励值
        ...
        """
        # 使用策略梯度算法计算损失
        log_prob = self.model.get_log_prob(xh_lig_hat)  # 计算对数概率
        ddpo_loss = -log_prob * reward  # 使用 REINFORCE 算法
        return ddpo_loss
    def my_solving_the_other_atom(self, ligand, pocket):
        ligand['one_hot'] = ligand['one_hot'][:, :-1]
        pocket['one_hot'] = pocket['one_hot'][:, :-1]

    def forward(self, data):
        print(f"[INFO]: lightning_modules.py: This is for training, forwarding ...")
        ligand, pocket = self.get_ligand_and_pocket(data) #将输入数据 data 分离为配体（ligand）和口袋（pocket）两部分
        # see summary.txt there are 11 kinds of atoms (include 'other'), but the constants.py do not use this!!!
        self.my_solving_the_other_atom(ligand, pocket)
        # print(ligand['one_hot'].size())
        ...
        # 前向过程：生成加噪后的数据和损失信息
        # lightning_modules 的 forward 函数把处理好的配体 ligand 和口袋 pocket 数据传递给了 
        # conditionalmodel 中的 forward 函数进行处理，以获取加噪后的数据和损失信息。
        ...
        print("back to lightening for training...")
        #  根据损失类型和训练状态计算损失
        if self.loss_type == 'l2' and self.training: # 就是L2
        ...
        # VLB objective or evaluation step
            ...
            print('[INFO]lightning_modules: Note, we are using SNR_weight')
        ...
        # 尝试添加 DDPO 的算法，使用机器学习优化参数信息：
        '''
        if self.training and self.use_ddpo:  # 仅在训练时使用 DDPO
            reward = self.compute_reward(xh_lig_hat, ligand, pocket)  # 计算奖励
            ddpo_loss = self.compute_ddpo_loss(xh_lig_hat, reward)  # 计算 DDPO 损失
            nll = nll + ddpo_loss  # 将 DDPO 损失添加到总损失中
        '''
        return nll, info
    ...
    def generate_ligands(self, pdb_file, n_samples, pocket_ids=None,
                         ...
                         n_nodes_bias=0, n_nodes_min=0,pdb_id = None,optimize=None,path=None,path_save=None,svdd=0,spsa = 0, **kwargs):# 额外传入pdb_id参数
        """
        ...
        Args:
            ...
            num_nodes_lig: number of ligand nodes for each sample (list of
                integers), sampled randomly if 'None' 每个样本的配体节点数量（整数列表），如果为 None 则随机采样。
        ...
        pocket:
        x:重复后的口袋坐标。
        one_hot:重复后的口袋类型的独热编码。
        size:每个重复的口袋大小。
        mask:用于区分不同重复的口袋。
        """
        print("[INFO] lightning_modeles.py:generate_ligands")
        ...
        # Pocket's center of mass 计算口袋的质心 [8.6390, -33.6489, -33.4662]*20(样本数)
        pocket_com_before = scatter_mean(pocket['x'], pocket['mask'], dim=0)
        
        # Create dummy ligands  确定配体节点数量
        ...
        # Apply minimum ligand size 确保每个样本的配体节点数量不小于 n_nodes_min。
        num_nodes_lig = torch.clamp(num_nodes_lig, min=n_nodes_min)

        #//////////////////////////////////diffusion方法的接入口////////////////////////////////////////
        # Use inpainting
        if type(self.ddpm) == EnVariationalDiffusion:
            print("[INFO]:lightning.py:  we are in inpainting")
        ...
        # Use conditional generation
            ...
            print("we are in ConditionalDDPM")
            xh_lig, xh_pocket, lig_mask, pocket_mask = \
                self.ddpm.sample_given_pocket(pocket, num_nodes_lig,
                                              timesteps=timesteps,pocket_com_before=pocket_com_before,dataset_info=self.dataset_info,sanitize=sanitize,relax_iter=relax_iter,largest_frag=largest_frag, pdb_id = pdb_id, device = self.device,optimize=optimize,path=path,path_save=path_save,svdd=svdd,spsa = spsa)
        ...
        # Build mol objects
        x = xh_lig[:, :self.x_dims].detach().cpu() # 提取xh_lig 中的原子坐标部分。 表示选取 xh_lig 中所有行（第一个 :），以及从第 0 列到第 self.x_dims - 1 列（第二个 :self.x_dims）的元素。
        atom_type = xh_lig[:, self.x_dims:].argmax(1).detach().cpu() # atom_type 提取了 xh_lig 中的原子类型信息，通过 argmax(1) 找到每个原子类型的独热编码中最大值的索引，即原子类型的编号。
        lig_mask = lig_mask.cpu()
