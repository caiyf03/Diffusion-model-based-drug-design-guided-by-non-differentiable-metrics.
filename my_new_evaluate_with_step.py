...
# 定义文件夹路径
#folder_path = 'D:\\school\\bishe\\DiffSBDD\\my_example'
folder_path = 'DiffSBDD/my_example'
# 获取文件夹下的所有文件
folder_files = [os.path.join(folder_path, file) for file in os.listdir(folder_path) if file.endswith('.sdf')]

print(f'in')
# 打开一个txt文件用于写入数据
with open('my_new_evaluation_results.txt', 'w', encoding='utf-8') as f:
    # 写入表头
    ...
    for file_path in folder_files:
        ...
        if result:
            ...
            # 将结果写入文件
            f.write(f"step {number}: QED: {np.mean(qed_flattened):.3f} +/- {np.std(qed_flattened):.2f}, SA: {np.mean(sa_flattened):.3f} +/- {np.std(sa_flattened):.2f},LogP: {np.mean(logp_flattened):.3f} +/- {np.std(logp_flattened):.2f},\
