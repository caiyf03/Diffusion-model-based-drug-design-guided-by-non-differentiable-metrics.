...
if __name__ == '__main__':
    ...
    if args.outdir is None:
        ...
        processed_dir = Path(args.basedir, 'my_new_', f'processed{suffix}')
    else:
    ...
    for split in data_split.keys():
        if split == 'train':
            # 计算训练集新的大小，向上取整
            new_size = math.ceil(len(data_split[split]) / 10)
            data_split[split] = random.sample(data_split[split], new_size)
        else:
            data_split[split] = random.sample(data_split[split], len(data_split[split]) // 1)
    # ... existing code ...
    #####################################################################################################################
    #####################################################################################################################
